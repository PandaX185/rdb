...
namespace core
{
    ...
    bool Store::lpush(const std::string &key, const std::string &value)
    {
        ...
        if (it == impl.data.end())
        {
            impl.data.emplace(key, std::deque<std::string>{value});
            return true;
        }
    ...
    std::optional<std::deque<std::string>> Store::lrange(const std::string &key, int start, int end)
    {
        ...
        if (it != impl.data.end() && it->second.type == ValueType::LIST)
        {
            ...
            if (start < 0) start = 0;
            if (end >= static_cast<int>(list.size())) end = list.size() - 1;
            if (start > end) return std::deque<std::string>{};
    ...
    bool Store::sadd(const std::string &key, const std::string &value)
    {
        ...
        if (it == impl.data.end())
        {
            impl.data.emplace(key, std::unordered_set<std::string>{value});
            return true;
        }
